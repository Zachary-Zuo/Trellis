 ...
 *   1. Version match between `@zachary/trellis` and
 *      `@zachary/trellis-core` (and the current git tag when checked from
 *      a tag context).
 ...
 *                                    on @zachary/trellis-core resolves
...
function verifyPackedCli() {
  ...
  try {
    ...
    const dep = packedPkg.dependencies?.["@zachary/trellis-core"];
    if (!dep) {
      fail(`packed CLI is missing dependency on @zachary/trellis-core.`);
    }
    if (dep !== v.cliVersion) {
      fail(
        `packed CLI depends on @zachary/trellis-core@"${dep}" but expected exact "${v.cliVersion}".\n` +
          `pnpm should rewrite workspace:* to the exact published version; got "${dep}" instead.`,
      );
    }
    console.log(
      `${GREEN}ok${RESET} packed CLI pins @zachary/trellis-core to exact ${v.cliVersion}.`,
    );
  } finally {
